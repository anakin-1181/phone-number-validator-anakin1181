...
def test_phone_number_without_country_code(validator, requests_mock):
    requests_mock.get(
        validator.api_url + PHONE_NUMBER_WITHOUT_COUNTRY_CODE,
        json={"valid": True, "country_code": "US"},
    )
    assert (
        validator.validate(PHONE_NUMBER_WITHOUT_COUNTRY_CODE, country_code="US") == True
    )
